<template>
  <div class="container m-auto flex justify-center pt-6 pb-3">
    <ul class="flex flex-wrap justify-between items-stretch">
      <li>
        <nuxt-link :to="{name: $route.name, query: {page: 1}}" class="p-2 bg-gray-300">
          &lt;&lt;
        </nuxt-link>
      </li>
      <li v-for="(paginatorItem, index) in paginator.start" :key="index">
        <nuxt-link :to="{name: $route.name, query: {page: paginatorItem}}" class="p-2 bg-gray-300">
          {{ paginatorItem }}
        </nuxt-link>
      </li>
      <li>
      ...
      <li>
        <nuxt-link :to="{name: $route.name, query: {page: $store.state.cdn.totalPages}}" class="p-2 bg-gray-300">
          &gt;&gt;
        </nuxt-link>
      </li>
    </ul>
